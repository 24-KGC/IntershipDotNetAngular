...
recipes.MapGet("/", async (ClaimsPrincipal user, AppDbContext db) =>
{
    var userId = GetUserId(user);
    if (userId is null)
    {
...
recipes.MapPost("/", async (CreateRecipeRequest request, ClaimsPrincipal user, AppDbContext db) =>
{
    var userId = GetUserId(user);
    if (userId is null)
    {
...
recipes.MapGet("/{id}", async (string id, ClaimsPrincipal user, AppDbContext db) =>
{
    var userId = GetUserId(user);
    if (userId is null)
    {
...
recipes.MapPut("/{id}", async (string id, UpdateRecipeRequest request, ClaimsPrincipal user, AppDbContext db) =>
{
    var userId = GetUserId(user);
    if (userId is null)
    {
...
recipes.MapDelete("/{id}", async (string id, ClaimsPrincipal user, AppDbContext db) =>
{
    var userId = GetUserId(user);
    if (userId is null)
    {
...
static IResult? ValidateRecipeInput(string title, string ingredients, string tag, int priority, int cookTime, int numSteps)
{
    var errors = new Dictionary<string, string[]>();
...
static RecipeResponse ToRecipeResponse(RecipeItem recipe)
{
...
static int CountIngredients(string ingredients)
{
    if (string.IsNullOrWhiteSpace(ingredients))
    {
        return 0;
